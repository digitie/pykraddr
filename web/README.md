...
기본 주소는 `http://localhost:3010`입니다. 전체 목록 탭을 로컬 DB와 연결해 확인할
때만 선택 어댑터를 `http://localhost:3011`에서 실행합니다.
...
`전체 목록` 탭은 `backend/`의 선택 어댑터에서 PostGIS 주소 목록을 받아옵니다.
외부 시스템 연동 기준은 이 어댑터가 아니라 `kraddr.geo` Python 라이브러리입니다.
로컬 샘플 4건이 아니라 `address_serving_juso_road_address` 테이블을 페이지 단위로
브라우징합니다.
